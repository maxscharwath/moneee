...
import {PopoverAnchor, Popover} from '@/components/ui/popover.tsx';
import * as PopoverPrimitive from '@radix-ui/react-popover';

const LongPressContext = createContext<{longPressState: 'normal' | 'pending' | 'pressed'; attrs: LongPressFns} | null>(null);

const useLongPressContext = () => {
	const context = useContext(LongPressContext);
	if (!context) {
		throw new Error('useLongPressContext must be used within a LongPressProvider');
	}
...
export const Root: React.FC<React.PropsWithChildren> = ({children}) => {
	const [longPressState, setLongPressState] = useState<'normal' | 'pending' | 'pressed'>('normal');
	...
	return (
		<LongPressContext.Provider value={{longPressState, attrs}}>
...
export const Trigger: React.FC<React.PropsWithChildren<{asChild?: boolean}>> = ({asChild, children}) => {
...
export const Content: React.FC<React.ComponentPropsWithoutRef<typeof PopoverContent>> = props => (
