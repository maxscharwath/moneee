...
const indicatorContainerVariants = cva('pointer-events-none relative', {
    variants: {
        position: {
            default: '',
            'top-right':
                'absolute -translate-y-1/2 translate-x-1/2 right-1 top-1',
            'top-left':
                'absolute -translate-y-1/2 -translate-x-1/2 left-1 top-1',
            'bottom-right':
...
export const Indicator = ({
    ...
    return (
        <div
            className={cn(indicatorContainerVariants({ position }), className)}
        >
            {!noPing && (
                <div className={cn(indicatorPingVariants({ variant }))} />
            )}
            <div
