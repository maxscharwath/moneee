...
export function usePeriod(initialPeriod: PeriodType = "monthly") {
	...
	const getPeriodDates = useMemo(
		() => calculatePeriodDates(currentPeriod, periodType),
		[currentPeriod, periodType],
	);

	const nextPeriod = () =>
		setCurrentPeriod((prevDate) => shiftPeriod(prevDate, periodType, "next"));
	const previousPeriod = () =>
		setCurrentPeriod((prevDate) =>
			shiftPeriod(prevDate, periodType, "previous"),
		);

	const getPreviousPeriodDates = useMemo(() => {
		const previousPeriodDate = shiftPeriod(
			currentPeriod,
			periodType,
			"previous",
		);
